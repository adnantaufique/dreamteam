...
- **S67** — the conductor spends the run's wall-clock where the work is: a gate's whole panel — statics + uncontended verifiers — goes out as one truthfully-announced wave ("gate wave: …, parallel", or the degraded form + its reason where the platform or caps forbid parallel — printing ", parallel" falsely is itself a violation) under `max_concurrent_agents` with only genuinely contended verifiers serialized per the resource split, independent workstream prep may overlap a running gate, independent conductor tool calls batch into one step (dependent calls stay sequential), each leaf's return contract — status, verdict + rationale, findings, facts, skill-usage line — travels verbatim while only its free narrative digests to a ledger line (a paraphrased finding is a violation), the report buffer may live in a per-run run-scratch file — outside the repo tree, uniquely named, created fresh, deleted only after report + retro (stateless across runs, S44's spirit intact) — and a one-workstream run reports on one screen carrying §Report's full content set — format shrinks, content and checks never.
