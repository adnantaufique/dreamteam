...
## Conductor economy (S67)

- **S67** — the conductor spends the run's wall-clock where the work is: a gate's whole panel — statics + uncontended verifiers — goes out as one truthfully-announced wave ("gate wave: …, parallel", or the degraded form + its reason where the platform or caps forbid parallel — printing ", parallel" falsely is itself a violation) under `max_concurrent_agents` with only genuinely contended verifiers serialized per the resource split, independent workstream prep may overlap a running gate, independent conductor tool calls batch into one step (dependent calls stay sequential), each leaf's return contract — status, verdict + rationale, findings, facts, skill-usage line, evidence pointers — travels verbatim while only its free narrative digests to a ledger line (a paraphrased finding is a violation), the report buffer lives in a per-run run-scratch file (mandatory from the run's first unit return) — outside the repo tree, uniquely named, created fresh, deleted only after report + retro (stateless across runs, S44's spirit intact) — and a one-workstream run reports on one screen carrying §Report's full content set — format shrinks, content and checks never.

## Resilience adds (S68)

- **S68** — past a compaction boundary (deliberate `/compact` or auto) the conductor re-reads `SKILL.md` from its on-disk home and the run-scratch ledger — mandatory from the run's first unit return; no ledger → auto-compact recovery from disk ground truth + the compaction summary — and re-prints the re-anchor line before the next artifact action (prose on all six CLIs; on Claude Code a default-on, fail-open, `DREAMTEAM_NO_COMPACT_REMINDER=1`-silenceable `SessionStart(compact)` hook additionally injects the one-line reminder — advisory, distinct from the opt-in deny layer); an over-bound leaf (default 3× the role's median wall this run, never tighter than the stated 30-minute no-baseline absolute) is checked for progress first, then killed and re-dispatched once via the dispatch-failure path or escalated — doubt escalates, never a silent kill, never an unbounded await wherever the platform permits action (OpenCode's synchronous dispatch: post-hoc, stated) — while a dispatch declared long in its brief (the declaration ledger-recorded at dispatch, surviving a re-arm) doesn't trip the watchdog, and the reliability tally carries per-role median + max durations the retro can cite, ledger/report only with no new store.
...
- **S70** — an operational DO-intent task ("upload and run a Colab notebook, monitor until done, download the results") classes `operate` at entry — no wrapper, no brainstorm, no plan doc (the operation is the spec), the class declared in the re-anchor line (`entry operate: completion criterion = …`, no task-scale tag) with any operate-vs-build doubt going to the build path: one operator producer dispatched with the completion criterion, the connected capability line, and a declared-long watchdog bound; the conductor never drives the service itself, the gate inspects the deliverable artifact plus captured completion evidence and never re-executes a side-effectful or paid operation (build/test re-run semantics untouched), the panel stays risk-scaled and widens past a manifest its risk class outgrew (a production restart classes high → the conductor widens the RC-only panel per the risk-scaling guard, under the entry class's light ceremony), and with no capability and no capable agent the existing escalate terminal presents "do it manually" as one option among others — never the default for a routable task.

## Closed learning loop (S71)

- **S71** — a consult that sways a cast must be cited and is recorded at cast (an applied-row line in the run-scratch ledger) and persisted by the retro — `applied_runs` + `outcome_when_applied` per learnings row (integer totals, exact arithmetic), plus a per-profile baseline accumulating only runs where no learnings row was applied (an applied run is excluded — never its own anchor); at ≥3 applied runs confidence moves by arithmetic against a grown baseline (same-statistic fractions compared cross-multiplied; the feedback agent annotates but cannot flip the direction; a baseline that hasn't grown → the confidence holds), so a learning that never beats a still-growing baseline decrements to the drop floor and is removed; a ≥0.5 persist also emits a per-install learned scenario (`tests/learned-scenarios.md`, S1001+, never shipped) the runner executes via `-File`/`--file learned` — human gate, evidence rule, and cross-run statelessness unchanged, and the track record is advisory, not causal proof.
