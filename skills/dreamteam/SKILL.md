...
- **A cap-STOP escalates with options, in plain language.** When a run stops at a mechanical cap — fix-loop exhaustion (above), `max_total_dispatches`, or a budget-exhaustion STOP under budget-aware scaling (`references/caster.md`) — the escalation is presented in the **same four-part shape as a User-Challenge (§Autonomy, where the frame is defined)**, in ordinary words: (1) what happened and why the run stopped, (2) the recommendation + why, (3) **2–3 concrete options, each with its cost** (e.g. raise the cap and continue · split the task into smaller workstreams · take the verified partial result and stop), (4) **the default if the user just says "continue"**. An extension of the existing frame, **not a new pause mechanism** — the STOP above already pauses; this frames what the user sees. An option may **RAISE a cap** and resume; **no option ever lowers the gate** ("never weaken the bar to force a pass" is untouched).
