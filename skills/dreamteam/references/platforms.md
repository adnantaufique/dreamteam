...
- **Effort dial.** Codex exposes `model_reasoning_effort`; on Claude Code, **Workflow-mode `agent()` exposes an `effort` option** — the Agent tool (background-subagent dispatch) has none, and its absence is fine, never an error. The dial is an extra lever **on top of** the tier, never a substitute for it — and **downward-only at dispatch**: where the table's `max` rung *is* capable + high effort, raising effort on a `capable` role lands on `max` (the escalation ladder's top — escalation or explicit override, never a casual dispatch-time raise); per-role guidance (which roles may run low, which never drop below the platform default) lives in the tier rubric (`references/caster.md` §"Model tier").
